#!/usr/bin/env bash
# Auto-approve routine prompts on a shared OpenCode server; leave flagged prompts for the user.
set -u
...
LOG_FILE="${OPENCODE_PERMISSION_MONITOR_LOG:-$STATE_DIR/permission-decisions-$TODAY.md}"
INTERVAL="${OPENCODE_PERMISSION_MONITOR_INTERVAL:-4}"
STATE_FILE="${OPENCODE_PERMISSION_MONITOR_STATE:-$STATE_DIR/permission-monitor.seen}"
...
ensure_log() {
    [[ -s "$LOG_FILE" ]] && return
    {
        ...
        printf 'Policy: allow liberally for routine development and local disk access; leave requests that look like destructive disk operations or credential disclosure pending for the user to approve or reject. Token-like values are redacted before logging.\n\n'
        printf '## Decisions\n'
    } > "$LOG_FILE"
}
...
secret_path_regex='(^|/)(\.env[^/]*|id_rsa|id_ed25519|\.gnupg|\.ssh|\.aws|\.kube|\.password-store|Library/Keychains)(/|[[:space:]"'"'"']|$)|\.(pem|key)([[:space:]"'"'"']|$)'
...
decide_legacy() {
    ...
    if [[ "$permission" == "read" || "$permission" == "edit" ]]; then
        if [[ "$text" =~ $secret_path_regex ]]; then
            printf '%s\t%s\n' defer 'secret-bearing file path'
        else
    ...
    if [[ "$permission" == "bash" ]]; then
        if is_destructive_command "$text"; then
            printf '%s\t%s\n' defer 'destructive disk operation'
        elif [[ "$text" =~ $secret_regex ]]; then
            printf '%s\t%s\n' defer 'possible secret disclosure'
        else
...
append_log() {
    local location="$1" req="$2" reply="$3" reason="$4" api_status="$5"
...
generate_candidate_summary() {
    ...
    printf 'Generated from reply attempts in this note, including failed replies. Review before adapting to opencode.jsonc; counts are attempts, not successful approvals.\n'
    for bucket in 'Allowed' 'Rejected'; do
...
reply_legacy() {
    ...
    api_curl -sS --max-time "$REPLY_TIMEOUT" -o /dev/null -w '%{http_code}' -X POST "$url" \
...
reply_v2() {
    ...
    api_curl -sS --max-time "$REPLY_TIMEOUT" -o /dev/null -w '%{http_code}' -X POST "$BASE_URL/api/session/$session_id/permission/$request_id/reply" \
        -H 'content-type: application/json' \
        --data "$(jq -nc --arg reply "$reply" --arg message "$reason" '{reply:$reply,message:$message}')"
}
...
legacy_requests() {
    ...
    api_curl -fsS --max-time "$LIST_TIMEOUT" "$url" 2>/dev/null || printf '[]'
}
...
v2_requests() {
    ...
    api_curl -fsS --max-time "$LIST_TIMEOUT" "$url" 2>/dev/null || printf '{"data":[]}'
}
...
process_legacy_location() {
    ...
    jq -c '.[]?' <<< "$requests" | while IFS= read -r req; do
        ...
        if [[ "$reply" == "defer" ]]; then
            say "v1 ${directory:-<default>} $id -> pending user decision ($reason) :: $summary"
            continue
        fi
        http_status="$(reply_legacy "$id" "$directory" "$reply" "$reason")"
        say "v1 ${directory:-<default>} $id -> $reply ($reason) status=$http_status :: $summary"
        append_log "${directory:-<default>}" "$req" "$reply" "$reason" "$http_status"
...
process_v2_location() {
    ...
    jq -c '.data[]?' <<< "$requests" | while IFS= read -r req; do
        ...
        if [[ "$reply" == "defer" ]]; then
            say "v2 ${directory:-<default>} $id -> pending user decision ($reason) :: $summary"
            continue
        fi
...
directories_from_api() {
    api_curl -fsS --max-time "$LIST_TIMEOUT" "$BASE_URL/api/session?limit=500&order=desc" 2>/dev/null |
...
for duration in "$INTERVAL" "$LIST_TIMEOUT" "$REPLY_TIMEOUT"; do
    if ! [[ "$duration" =~ ^[0-9]+([.][0-9]+)?$ ]] || ! awk -v value="$duration" 'BEGIN { exit !(value > 0) }'; then
        printf 'Intervals and timeouts must be positive numbers: %s\n' "$duration" >&2
        exit 64
    fi
done
BASE_URL="${BASE_URL%/}"
...
if [[ "$DRY_RUN" -eq 1 ]]; then
    say "starting dry-run; no API replies, logs, or state will be written"
else
    umask 077
    mkdir -p "$(dirname "$STATE_FILE")" "$(dirname "$LOG_FILE")" || exit 1
