...
function primaryContextPath(): string {
  if (process.env.OPENCODE_PRIMARY_CONTEXT_FILE) return process.env.OPENCODE_PRIMARY_CONTEXT_FILE
...
function readPrimaryContext(path = primaryContextPath()): string | undefined {
  try {
    const context = readFileSync(path, "utf8").trim()
...
async function isRootSession(client: PluginInput["client"], sessionID: string | undefined): Promise<boolean> {
  if (!sessionID) return false

  const cached = rootSessionCache.get(sessionID)
  if (cached !== undefined) return cached

  const response = await withTimeout(client.session.get({ path: { id: sessionID } }), SESSION_LOOKUP_TIMEOUT_MS)
  const isRoot = response?.data !== undefined && response.data.parentID == null
...
const testHelpers = Object.freeze({ isRootSession, primaryContextPath, readPrimaryContext })
